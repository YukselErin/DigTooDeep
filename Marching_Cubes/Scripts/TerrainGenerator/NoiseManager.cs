...
public class NoiseManager : Singleton<NoiseManager>
{
	...
	{
		restartAmount++;
		worldConfig.worldSeed += restartAmount;
	...
	public static float[] GenenerateSimple3DNoiseMap(float scale, Vector2Int offset, int ore = 0)
	{
		float[] noiseMap = new float[Constants.CHUNK_VERTEX_AREA * Constants.CHUNK_VERTEX_HEIGHT];//Size of vertex + all next borders (For the slope calculation)

		System.Random random = new System.Random(NoiseManager.Instance.worldConfig.worldSeed + ore * 10);//Used System.random, because unity.Random is global, can cause problems if there is other random running in other script
		...
		for (int z = 0; z < Constants.CHUNK_VERTEX_SIZE; z++)
		{
			for (int x = 0; x < Constants.CHUNK_VERTEX_SIZE; x++)
			{
				...
				{
					float sampleX = (x - halfVertexArea + offsetX) / scale;
					float sampleZ = (z - halfVertexArea + offsetY) / scale;
					float sampleY = (y - halfVertexArea + offsetY) / scale;

					noiseMap[x + z * Constants.CHUNK_VERTEX_SIZE + y * Constants.CHUNK_VERTEX_AREA] = PerlinNoise3D(sampleX, sampleY, sampleZ);
				}
